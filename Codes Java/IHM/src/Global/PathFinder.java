...
public class PathFinder {
	...
	public static void moveDrones() {
		if(grid.drones.isEmpty())
			return;
		
		Path p = grid.path;
		
		while(p.hasNext()) {
			if(grid.drones.remove(p.element)) {
				Tile next = p.next.element;
				Drone nouveau = new Drone(next.x, next.y);
				grid.drones.add(nouveau);
				grid.tilesGrid[nouveau.x][nouveau.y] = nouveau;
				grid.toRepaint.add(nouveau);
			}
			p = p.next;
		}

...
public class Controller {
	...
	public static double p = 0.9; // Probabilité de faire le choix optimal
	...
	public static void init() {
		...
		for (int i = 0 ; i < 50 ; i++ ) {map.addCheckpoint(new Checkpoint(Math.random() * width * 0.3, Math.random() * height * 0.3));}
		for (int i = 0 ; i < 50 ; i++ ) {map.addCheckpoint(new Checkpoint(Math.random() * width * 0.3 + width * 0.7, Math.random() * height * 0.3));}
		for (int i = 0 ; i < 50 ; i++ ) {map.addCheckpoint(new Checkpoint(Math.random() * width * 0.3, Math.random() * height * 0.3 + height * 0.7));}
		for (int i = 0 ; i < 50 ; i++ ) {map.addCheckpoint(new Checkpoint(Math.random() * width * 0.3 + width * 0.7, Math.random() * height * 0.3 + height * 0.7));}
		...
		for (int i = 0 ; i < 4 ; i++) {drones.add(new Drone(Math.random() * width, Math.random() * height));}
	...
	public static void makeDecision(int droneIndex) {
		...
		// Le drone à une probabilité p de choisir le checkpoint le plus intéressant, et 1-p d'en choisir un au hasard
		int indexMaxInterest = -1;
		double maxInterest = 0;
		double interest;
		for (int j = 0 ; j < map.getCheckpoints().size() ; j++) { // On récupère le checkpoint le plus intéressant
		...
		if (random < p) { // On choisit le checkpoint le plus intéressant
		...
		for (int i = 0 ; i < drones.size() ; i++) {
			if (droneTargets[i] == indexMaxInterest & i != droneIndex) { // On est dans le cas d'un conflit
				if (drones.get(droneIndex).getPosition().distance(checkpointMaxInterest.getPosition()) > drones.get(i).getPosition().distance(checkpointMaxInterest.getPosition())){
					// Le conflit est perdu, on élimine ce checkpoint des checkpoints autorisés et on retente
					availableCheckpoints[droneIndex][indexMaxInterest] = false;
					makeDecision(droneIndex);
					return;
				}
				else { // Le conflit est gagné, on fait plutôt choisir le drone d'indice i

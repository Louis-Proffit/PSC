...
public class Genetique implements Comparator<ArrayList<Integer>> {
	...
	LinkedList<? extends Tile> points = Test.grid.checkpoints;
	...
	public LinkedList<Tile> geneticPathFinder(){
		...
		for (int i = 0 ; i < numberOfCheckpoints; i++) {
			...
			for (int j = 0 ; j < numberOfCheckpoints; j++) {
				...
				LinkedList<AstarTile> chemin = AstarBis.aStar(Test.grid, new AstarTile(points.get(i)), new AstarTile(points.get(j)));

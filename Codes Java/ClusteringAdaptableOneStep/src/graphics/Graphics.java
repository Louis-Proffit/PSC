...
public class Graphics {
    ...
    public void updateGraphics(ClusterAttribution clusterAttribution) {
        ...
        ArrayList<Drone> drones = clusterAttribution.getDrones();
     ...
     */
    private void updateGraphicsForDrone(Drone drone, ArrayList<Checkpoint> cluster) {
        paintDrone(drone);
     ...
     */
    private void paintDronePath(ArrayList<Checkpoint> checkpoints) {
        draw.setPenRadius(0.01);
        int size = checkpoints.size();
        if (checkpoints.size() == 0)
            return;
        if (checkpoints.size() == 1) {
            paintCheckpoint(checkpoints.get(0));
            return;
        }
        ...
        paintCheckpoint(checkpoints.get(size - 1));
        paintLine(checkpoints.get(size - 1), checkpoints.get(0));
    }

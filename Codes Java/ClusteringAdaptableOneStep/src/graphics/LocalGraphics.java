...
public class LocalGraphics implements MouseListener, KeyListener, GraphicsInterface {
	...
	public void updateGraphics(ClusterAttribution clusterAttribution) {
		...
		List<Drone> drones = clusterAttribution.getDrones();
	 ...
	 */
	private void updateGraphicsForDrone(Drone drone, List<Checkpoint> cluster) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (mouseState == MouseState.CHECKPOINT) {
			Controller.addCheckpoint(ImproveType.COMPLETE, new Checkpoint(vector));
		} else if (mouseState == MouseState.DRONE) {
			Controller.addDrone(ImproveType.COMPLETE, new Drone(vector));
		}
	}

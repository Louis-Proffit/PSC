package graphics;

import java.awt.Dimension;

import javax.swing.JFrame;

// Cadre de l'objet graphique
public class Frame extends JFrame {

	private static final long serialVersionUID = 1L;

	// Champ fen�tre
	public Window window;

	// Constructeur d'un cadre � partir de sa largeur, sa hauteur et de la grille
	// grid. D�l�gue au constructeur de Window
	public Frame(int width, int height, LocalGraphics localGraphics) {
		this.setTitle("Simulation");
		this.window = new Window(width, height, localGraphics);
		this.setSize(new Dimension(width, height));
		this.getContentPane().add(window);
		this.addKeyListener(window);
		this.addMouseListener(window);
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		this.setResizable(false);
		this.setLocationRelativeTo(null);
		this.setLayout(null);
		this.setVisible(true);
	}
}

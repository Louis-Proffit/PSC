...
int main2() {
	Drone d = Drone();
	Drone e = Drone();
	vec3 p = d.get_position();
	vec3 q = e.get_position();
	vec3 o = vec3( 0,0,0);
	std::cout << p.x << " " << p.y << " " << p.z << std::endl;
	std::cout << q.x << " " << q.y << " " << q.z << std::endl;
	std::cout << d.distance_to(q) << std::endl;
	return 0;
}
...
int main(int, char* argv[])
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		display_interface();
		update_graphics();
		update_statistics(); // mise à jour du fichier statistiques
...
void update_distance() {
	double a = drones[0].distance_to(drones[1].get_position());
	double b = drones[0].avg_distance(drones, number_of_drones);
	
	stats.update_dist(a);
	//stats.update_avg_dist(b);
